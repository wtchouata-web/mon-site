...
export default function App() {
  ...
  const [simulatedInboxData, setSimulatedInboxData] = useState<{ email: string; userId: string; name?: string } | null>(null);
  ...
  // --- AUTOMATIC VISITOR SECURITY AUDIT INGESTION & EMAIL VERIFICATION PARSING ---
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const verifyUserId = params.get('verify_user_id');
    const simulateEmail = params.get('email');
    const simulateUserId = params.get('user_id');
    const simulateName = params.get('name');
    const simulateInbox = params.get('simulate_inbox');

    if (verifyUserId) {
      // Clean up local storage and parse
      const users: User[] = JSON.parse(localStorage.getItem('rose_amour_users') || '[]');
      const idx = users.findIndex(u => u.id === verifyUserId);
      if (idx !== -1) {
        users[idx].isVerified = true;
        localStorage.setItem('rose_amour_users', JSON.stringify(users));
        
        // Sync to Express backend Rose Amour JSON database immediately
        ...
        .then(() => pullFromServer())
        .catch(err => console.error("Error syncing verified user:", err));

        alert("Félicitations ! Votre adresse mail a été confirmée avec succès. Vous pouvez maintenant vous connecter à votre compte.");
        setIsAuthModalOpen(true);
        // Clean URL params without page reload
        window.history.replaceState({}, document.title, window.location.pathname);
      }
    } else if (simulateInbox === 'true' && simulateEmail && simulateUserId) {
      setSimulatedInboxData({ email: simulateEmail, userId: simulateUserId, name: simulateName || undefined });
      // Clean URL params without page reload
      window.history.replaceState({}, document.title, window.location.pathname);
    }
  ...
  const pushToServer = async () => {
    const token = localStorage.getItem('rose_amour_admin_token');
    // Block non-admins from pushing full state overwrites
  ...
  const logWhatsAppClick = (product: Product, prefilledMessage: string) => {
    ...
    // Securely sync WhatsApp click to public API
    fetch('/api/public/whatsapp-click', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ click: newLog })
    }).catch(err => console.error("Error syncing whatsapp click:", err));
  };
  ...
  const handleAddConnectionLog = (newLog: Omit<ConnectionLog, 'id' | 'loginTime'>) => {
    ...
    // Securely sync connection log to public API
    fetch('/api/public/log', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ log: fullLog })
    }).catch(err => console.error("Error syncing connection log:", err));
  };
  ...
  const handleAddComment = (productId: string, authorName: string, rating: number, content: string) => {
    ...
    // Sync via secure public comment endpoint
    fetch('/api/public/comment', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ comment: newComment })
    }).catch(err => console.error("Error syncing comment:", err));
  };
  ...
  const handlePaymentSuccess = (paymentData: Omit<Sale, 'id' | 'createdAt' | 'status'> & { verificationCode?: string; planType?: string }) => {
    ...
    let boostedProduct: Product | undefined;
    ...
    // Securely sync transaction and profile to backend
    fetch('/api/public/checkout-success', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sale: saleRecord, product: addedProduct || boostedProduct })
    }).catch(err => console.error("Error syncing payment checkout success:", err));
  };
  ...
  // --- EXTERNAL SIMULATED EMAIL INBOX FOR Independent SIGNUPS ---
  if (simulatedInboxData) {
    return (
      <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans">
        {/* Webmail Header bar */}
        <header className="bg-slate-950 border-b border-slate-800 px-4 md:px-8 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-rose-600 rounded-xl flex items-center justify-center text-white font-extrabold text-base">
              ✉️
            </div>
            <div>
              <h1 className="text-sm font-black tracking-tight text-white uppercase">Cameroun Secure Webmail</h1>
              <p className="text-[9px] text-rose-400 font-extrabold uppercase tracking-widest font-mono">Portail de Messagerie Indépendant</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden md:block text-right">
              <span className="block text-[10px] text-slate-400 font-bold">Compte actif</span>
              <span className="block text-xs font-semibold text-rose-300 font-mono">{simulatedInboxData.email}</span>
            </div>
            <button
              onClick={() => {
                if (confirm("Voulez-vous fermer le simulateur d'inbox ?")) {
                  setSimulatedInboxData(null);
                }
              }}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] font-bold rounded-lg transition-colors cursor-pointer"
            >
              Fermer ×
            </button>
          </div>
        ...
        <div className="flex-1 max-w-4xl w-full mx-auto p-4 md:p-8 flex flex-col gap-6">
          <div className="bg-slate-950 border border-slate-800 rounded-3xl p-5 md:p-8 shadow-xl relative overflow-hidden">
            <div className="absolute top-0 right-0 p-2 bg-rose-600 text-[8px] font-black uppercase tracking-widest rounded-bl-2xl">
              Nouveau Message
            </div>

            <div className="border-b border-slate-800 pb-4 space-y-2">
              <div className="flex items-center gap-2 text-xs">
                <span className="text-slate-400 font-bold w-16">De :</span>
                <span className="text-rose-300 font-extrabold font-mono">Rose Amour <span className="text-slate-500 font-normal">&lt;no-reply@rose-amour.cm&gt;</span></span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <span className="text-slate-400 font-bold w-16">À :</span>
                <span className="text-slate-300 font-bold font-mono">{simulatedInboxData.email}</span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <span className="text-slate-400 font-bold w-16">Objet :</span>
                <span className="text-white font-black text-sm">🌸 Confirmez votre inscription Rose Amour !</span>
              </div>
            </div>

            <div className="pt-6 space-y-5 font-sans leading-relaxed text-sm text-slate-200">
              <p className="font-extrabold text-rose-300 text-base">
                Bonjour {simulatedInboxData.name || 'Abonné(e)'},
              </p>
              
              <p>
                Nous vous remercions pour votre inscription sur <strong>Rose Amour</strong>, le premier réseau de mise en relation d'hôtesses de premier plan au Cameroun.
              </p>

              <div className="bg-slate-900 border border-rose-500/10 p-4 rounded-2xl my-4 space-y-2">
                <p className="text-xs text-rose-200 font-bold uppercase tracking-wider">🔒 PARAMÈTRES DE CONFIRMATION SÉCURISÉE :</p>
                <p className="text-xs text-slate-400">
                  Afin de valider définitivement votre compte indépendant et débloquer vos options d'approbation (Tarif fixe de validation de fiche), vous devez confirmer l'exactitude de votre adresse e-mail.
                </p>
              </div>

              <div className="py-4 text-center">
                <a
                  href={`${window.location.origin}/?verify_user_id=${simulatedInboxData.userId}`}
                  className="inline-block py-4 px-8 bg-rose-600 hover:bg-rose-700 active:scale-98 text-white font-black rounded-2xl shadow-lg transition-all text-xs uppercase tracking-widest cursor-pointer border border-rose-500 animate-pulse text-center"
                >
                  👉 CONFIRMER MON ADRESSE EMAIL ET RETOURNER AU SITE
                </a>
                <p className="text-[10px] text-slate-500 mt-2 font-mono uppercase tracking-widest">
                  Lien unique et confidentiel valable 5 jours
                </p>
              </div>

              <p className="text-xs text-slate-400 border-t border-slate-800/80 pt-4">
                Si vous n'êtes pas à l'origine de cette demande, veuillez ignorer ce message. Vos données seront automatiquement purgées sous 24h.
              </p>

              <p className="text-xs text-rose-400/80 font-bold font-mono">
                L'Équipe d'Administration Rose Amour.
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Footer info */}
        <footer className="bg-slate-950 py-4 text-center text-[10px] text-slate-500 border-t border-slate-900">
          Serveur Crypté Rose Amour &bull; Protection Anti-Fraude IP &bull; Douala, Cameroun
        </footer>
      </div>
    );
  }
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <span className="text-[10px] text-slate-500 block leading-tight">Bloquer l'accès à tous les membres et visiteurs du site, sauf aux administrateurs</span>
                              </div>
                              <button
                                onClick={() => {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {products
                        ...
                        .map(prod => (
                          <div 
                            key={prod.id}
                            className="relative h-[290px] rounded-3xl overflow-hidden shadow-lg border border-pink-500/35 group cursor-pointer flex flex-col justify-between"
                            onClick={() => setSelectedProduct(prod)}
                          >
                            {/* Full background photo of the Premium Hostess */}
                            <img 
                              src={prod.imageUrl} 
                              alt={prod.title} 
                            ...
                            />
                            {/* Strong gradient dark overlay to keep texts perfectly legible */}
                            <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-900/50 to-black/20" />

                            {/* Badge */}
                            <div className="absolute top-3 right-3 z-10 bg-pink-600 text-white font-black text-[8px] tracking-widest px-2.5 py-0.5 rounded-full shadow-md flex items-center gap-1 uppercase border border-pink-400/20">
                            ...
                            <div className="relative z-10 p-4 pt-10">
                              <span className="text-[9px] font-black text-pink-300 font-mono tracking-widest uppercase mb-0.5 block">
                                {prod.category}
                              </span>
                              <h4 className="text-sm font-extrabold leading-tight hover:text-rose-200 transition-colors line-clamp-1 font-sans text-white">
                                {prod.title}
                              </h4>
                              {prod.statusText && (
                                <div className="mt-1.5 text-[10px] bg-black/40 px-2 py-0.5 rounded-lg text-pink-200 italic border border-white/5 font-sans font-medium w-fit max-w-full truncate">
                                  " {prod.statusText} "
                                </div>
                              )}
                            </div>
                            ...
                            {/* Bottom info */}
                            <div className="relative z-10 p-4 bg-gradient-to-t from-black via-black/85 to-transparent">
                              <div className="flex justify-between items-end mb-2.5">
                                <div>
                                  <span className="text-[8px] text-rose-350/90 block uppercase tracking-wide">Tarif VIP</span>
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                <button
                                  onClick={(e) => { e.stopPropagation(); handleWhatsAppClick(prod, e); }}
                                  className="py-1 px-2 rounded-lg bg-pink-600 hover:bg-pink-750 text-white font-extrabold text-[10px] flex items-center justify-center gap-1 transition-all pointer cursor-pointer border border-pink-500"
                                >
                                ...
                                <button
                                  ...
                                  className="py-1 px-2 rounded-lg bg-white/10 hover:bg-white/20 text-white font-bold text-[10px] transition-colors cursor-pointer border border-white/10"
                                >
                                  Voir Profil
                                </button>
                              </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <label htmlFor="opt-boost" className="cursor-pointer">
                      <span className="block text-xs font-extrabold text-rose-950 flex items-center gap-1.5 leading-none">
                        Option Boost VIP Accueil Vitrine (+7 000 FCFA les 5 jours)
                      ...
                      <span className="block text-[10px] text-rose-700 mt-1.5 leading-relaxed font-light font-sans">
                        {newCategory === 'Modèles VIP' 
                          ? "En tant que 'Modèle VIP', ces frais de boost promotionnel de 7 000 FCFA sont requis et intégrés par défaut pour garantir votre visibilité Premium maximale."
