...
export default function CheckoutModal({
  ...
  const handleSubmitValue = (e: React.FormEvent) => {
    ...
    // Ensure the amount is secure and matches exactly the WooCommerce plan tariff
    ...
    if (isNaN(parsedAmount) || parsedAmount !== expected) {
      setTimeout(() => {
        setWooError(
          isFr
            ? `Erreur de paiement WooCommerce (Serveur sécurisé) : Le montant payé (${formatPrice(parsedAmount || 0)}) a été altéré ou ne correspond pas au tarif de ${formatPrice(expected)} exigé pour le plan ${selectedPlan === 'premium' ? 'Premium (7000 FCFA)' : 'Classique (5000 FCFA)'} !`
            : `WooCommerce Payment Error (Secure Server): The amount paid (${formatPrice(parsedAmount || 0)}) was modified or doesn't match the required price of ${formatPrice(expected)} for the ${selectedPlan} plan!`
        );
        setProcessing(false);
      }, 1000);
      return;
    ...
    // Server handshake: confirm exact amount and generate dynamic unique OTP code
    setTimeout(() => {
      const generatedOtp = Math.floor(100000 + Math.random() * 900000).toString();
      setWooOtpCode(generatedOtp);
      
      const generatedRefCode = generateUniqueCode();
      setGeneratedCode(generatedRefCode);
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {success ? (
            ...
            /* WOOCOMMERCE OTP VALIDATION PANEL (STEP 2) */
            <motion.div
              key="otp-screen"
            ...
            >
              <div className="text-center font-sans">
                ...
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight">
                  {isFr ? 'Validation de paiement WooCommerce' : 'WooCommerce Payment Validation'}
                </h3>
                <p className="text-[10px] text-slate-400 font-medium">
                  {isFr ? 'Le serveur de Rose Amour exige la saisie de votre OTP sécurisé' : 'Rose Amour server requires your secure OTP entry'}
                </p>
              </div>

              {/* Simulated SMS Alert Box with the Generated OTP */}
              <div className="p-4 bg-pink-50/50 border border-pink-100 rounded-2xl relative overflow-hidden text-left font-sans">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-orange-400 to-pink-500" />
                <span className="block text-[10px] font-black uppercase text-pink-800 tracking-wider mb-1 font-mono">
                  {isFr ? '💬 SMS REÇU (Gateway WooCommerce) :' : '💬 RECEIVED SMS (WooCommerce Gateway):'}
                </span>
                <p className="text-xs font-bold text-slate-700 leading-relaxed">
                  {isFr 
                    ? `[OTP CONFIDENTIEL] Votre code de confirmation pour le paiement de ${formatPrice(amount)} est : `
                    : `[CONFIDENTIAL OTP] Your verification code for the payment of ${formatPrice(amount)} is: `}
                  <strong className="text-sm font-mono text-pink-700 bg-white border border-pink-200 px-2 py-0.5 rounded ml-1 select-all select-text font-black tracking-wider">{wooOtpCode}</strong>.
                </p>
                <div className="text-[9.5px] mt-2 text-slate-450 font-medium font-sans">
                  {isFr 
                    ? `Destinataire : ${mobilePhone || 'Mon Numéro'}`
                    : `Recipient: ${mobilePhone || 'My Number'}`}
                </div>
              </div>
              ...
              <form onSubmit={handleVerifyOtpSubmission} className="space-y-4 text-left">
                <div>
                  <label className="block text-[10.5px] font-bold text-slate-500 uppercase tracking-wider mb-1 font-sans">
                    {isFr ? 'Saisir le code OTP reçu par SMS :' : 'Enter the OTP code received via SMS:'}
                  </label>
                  <input
                    type="text"
                    required
                    maxLength={6}
                    value={enteredWooOtp}
                    onChange={(e) => setEnteredWooOtp(e.target.value)}
                    placeholder="Ex: 123456"
                    className="w-full px-4 py-2 bg-slate-50 border border-slate-200 focus:border-pink-500 rounded-xl text-center text-sm font-black tracking-widest font-mono text-slate-800 focus:outline-hidden"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-2">
                  <button
                    ...
                    className="py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold rounded-xl text-xs uppercase cursor-pointer transition font-sans"
                  >
                    {isFr ? 'Retour' : 'Back'}
                  </button>
                  <button
                    type="submit"
                    className="py-2.5 bg-pink-600 hover:bg-pink-700 text-white font-extrabold rounded-xl text-xs uppercase cursor-pointer transition shadow-xs font-sans"
                  >
